...
type CIFailure struct {
	...
	Notes    string
	filename string // existing files keep their names so legacy notes need no migration
}

// Filename is keyed by workflow and check name rather than by a run id: the same check failing again
// on the next run has to land on the same file, or the notes from the last round have nothing
// to merge onto.
func (f CIFailure) Filename() string {
	if f.filename != "" {
		return f.filename
	}
	name := segment(f.Meta.Check)
	if len(name) > 80 {
		name = name[:80]
	}
	sum := sha256.Sum256([]byte(checkKey(f.Meta.Workflow, f.Meta.Check)))
	return fmt.Sprintf("ci-%s-%x.md", name, sum[:16])
}

func checkKey(workflow, name string) string { return workflow + "\x00" + name }
...
func (f *CIFailure) Merge(old CIFailure) {
	...
	// Notes explain previous attempts; they are not proof that this run passed.
	// Completion time also distinguishes a rerun of the same Actions run ID.
	if f.Meta.Bucket == bucketFail && (f.Meta.RunID != old.Meta.RunID || f.Meta.Link != old.Meta.Link ||
		(old.Meta.CompletedAt != "" && f.Meta.CompletedAt != old.Meta.CompletedAt)) {
		f.Meta.Status, f.Meta.Commit = StatusPending, ""
		return
	}
	if old.Fixed() {
