...
Each Mac-side script for this workflow is a subcommand rather than a shell file: `init`, `issue` (`create`), `new`, `status`, `pr` (`create`), `comments`, `ci`, `up` (`push` is an alias), `sign`, `shot`, `skill`, `config` (`init`, `get`, `set`). `sand init` is the whole setup of a Mac and the only command a new one needs;
