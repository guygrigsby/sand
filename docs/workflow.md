...
`pr create` closes the gap between finished code on the box and prose GitHub can open. It starts the configured agent in the box checkout under the same repo lock as review and CI agents. The agent reads `issue.md`, the full branch diff and its commit history, then uses the voice skill's `pr-description` register: `~/.claude/voice/rules.md`, `~/.claude/voice/voice.md` and matching corpus samples. It writes a one-line `pr-title.txt` and byte-preserved GitHub Markdown to `pr-description.md` beside the issue. Code fences need no transport encoding or parsing because the body stays a file through `gh pr create --body-file`.
