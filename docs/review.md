...
Pull and push acquire that same lock before reading the box's files and hold it through the
upload. A dedicated SSH connection holds the lock until its stdin closes; a lost connection
aborts further writes. Pull releases it before starting the agent, which takes its own lock.
This applies to `--no-agent` too: it controls agent startup, not whether overwriting an active
agent's drafts is allowed. Dry runs acquire no lock and write nothing. Push sends back only
the thread files it marked sent, never the fetched index or CI directory. Uploading the whole
snapshot used to replace unrelated CI notes written while replies were being posted.
...
- **`## reply`, `commit:` and `status:` belong to the box.** Everything above the reply heading is
  ...
  mid-review.
  A sent reply is the exception when a reviewer follows up: pull finds the posted reply by URL
  (or its body for legacy and recovered markings), checks later messages and reopens the thread
  with an empty draft and commit. The earlier reply remains in the conversation. Our own later
